      ...
      <div v-for="item in article" :key="item.id" class="dr-list">
        <div class="dr-top">
          <div style="overflow: hidden">
            <div class="dr-title" @click="editArticle(item.id)">{{ item.title }}</div>
          </div>
...
<script>
import { fetchAudit, deleteMyAudit } from '@/api/personal'
...
export default {
  ...
  methods: {
    getAudit() {
      fetchAudit(this.listQuery).then(response => {
    ...
    delArticle(id) {
      ElMessageBox.confirm(
        ...
        deleteMyAudit(this.articleId).then(() => {
          ElMessage({
            type: 'success',
            message: '删除成功',
          })
